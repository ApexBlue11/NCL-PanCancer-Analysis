...
EJMO requires 300-600 dpi; the submitted version used ~96 dpi screenshots in
which Figure 3a's labels were illegible and Figure 4c/4d carried no axis labels,
gene names or pathway names at all. Every panel here is drawn from the result
tables, with effect sizes, confidence intervals and sample sizes on the figure.
"""
import os
import numpy as np
import pandas as pd
...
def figure1():
    ...
    gs = fig.add_gridspec(2, 2, height_ratios=[1.55, 1], hspace=0.32, wspace=0.34)
    ...
    ax.legend(handles=[
        Line2D([], [], marker="o", ls="", color=UP, label="higher in tumour (q<0.05)"),
        Line2D([], [], marker="o", ls="", color=DOWN, label="lower in tumour (q<0.05)"),
        Line2D([], [], marker="o", ls="", color=NS, label="not significant"),
        Line2D([], [], marker="s", ls="", color=GREY, label="approximate tissue match"),
    ], fontsize=5.4, loc="lower right", frameon=False)
    ...
    # Distinguish the strong claim (significant in BOTH comparators yet opposite in
    # sign) from cases where only one comparator reaches significance. Colouring
    # every sign flip alike would overstate what the data show.
    ...
    if st is not None and len(st):
        ...
        ax.set_yticklabels([f"{r.cohort} (n={int(r.n_total)})" for _, r in s.iterrows()],
                           fontsize=5.6)
        ax.set_xlabel("Jonckheere–Terpstra z (stage I→IV trend)")
...
def figure2():
    ...
    gs = fig.add_gridspec(2, 2, hspace=0.36, wspace=0.32, height_ratios=[1.35, 1])
    ...
    ax.set_title("a", loc="left", fontweight="bold", fontsize=10)
    ax.annotate("† proportional-hazards assumption violated", (0.99, 0.02),
                xycoords="axes fraction", ha="right", fontsize=5.2, color=GREY)
    ...
    ax.set_title("b", loc="left", fontweight="bold", fontsize=10)
    ax.legend(fontsize=5.4, frameon=False)
    ...
    ax.bar(x + 0.19, adj, 0.38, color=ACCENT, label="adjusted")
...
def figure3():
    ...
    grid = fig.add_gridspec(2, 2, hspace=0.42, wspace=0.3, height_ratios=[1, 1])
    ...
    if gs_ is not None:
        ...
        y = np.arange(len(piv))
        for j, (m, mk) in enumerate([("Immune score", "o"), ("Stromal score", "s"),
                                     ("Microenvironment score", "^")]):
            if m in piv:
                ax.scatter(piv[m], y, s=11, marker=mk, alpha=0.8, label=m,
                           edgecolor="white", linewidth=0.2)
        ax.axvline(0, color="#222222", lw=0.7, ls="--")
        ax.set_yticks(y); ax.set_yticklabels(piv.index, fontsize=5.2)
...
def figure6():
    ...
    if qcol is None:
        print("  Figure 6 skipped: no FDR column")
        return
